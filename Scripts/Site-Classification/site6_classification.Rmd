...
```{r import data}
site6 <- rast(here("Data", "site6", "site6.tif")) #Import the site 1 orthomosaic
site6_poly <- vect(here("Data", "site6", "site6_poly.shp")) #Import the site 1 training polygons
```
...
for (i in 1:nrow(site6_poly)) {
  single_poly <- site6_poly[i, ] #Look in each individual polygon within the training polygons provided
  randomPts <- spatSample(single_poly, 200, method = "random") #Generate 200 randomly placed points within each polygon
  site6_pts <- rbind(site6_pts, randomPts) #Place these random points into the previously created empty spatvector
}
}
print(site6_pts) #Check to see if the spatvector contains the points 
```
...
pixel_res <- res(site6_classified)[1] #Obtain the pixel resolution
