...
```{r import data}
site2 <- rast(here("Data", "site2", "site2.tif")) #Import the site 1 orthomosaic
site2_poly <- vect(here("Data", "site2", "site2_poly.shp")) #Import the site 1 training polygons
```
...
for (i in 1:nrow(site2_poly)) {
  single_poly <- site2_poly[i, ] #Look in each individual polygon within the training polygons provided
  randomPts <- spatSample(single_poly, 200, method = "random") #Generate 200 randomly placed points within each polygon
  site2_pts <- rbind(site2_pts, randomPts) #Place these random points into the previously created empty spatvector
}
}
print(site2_pts) #Check to see if the spatvector contains the points 
```
...
pixel_res <- res(site2_classified)[1] #Obtain the pixel resolution
