...
```{r import data}
site3 <- rast(here("Data", "site3", "site3.tif")) #Import the site 1 orthomosaic
site3_poly <- vect(here("Data", "site3", "site3_poly.shp")) #Import the site 1 training polygons
```
...
for (i in 1:nrow(site3_poly)) {
  single_poly <- site3_poly[i, ] #Look in each individual polygon within the training polygons provided
  randomPts <- spatSample(single_poly, 200, method = "random") #Generate 200 randomly placed points within each polygon
  site3_pts <- rbind(site3_pts, randomPts) #Place these random points into the previously created empty spatvector
}
}
print(site3_pts) #Check to see if the spatvector contains the points 
```
...
pixel_res <- res(site3_classified)[1] #Obtain the pixel resolution
