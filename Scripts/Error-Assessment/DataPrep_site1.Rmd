...
Import all rasters from site 1
```{r load rasters}
site_1_1 <- rast(here("Data", "site1", "1_1_rast.tif"))
names(site_1_1) <- c("red", "green", "blue")
site_1_2 <- rast(here("Data", "site1", "1_2_rast.tif"))
names(site_1_2) <- c("red", "green", "blue")
site_1_3 <- rast(here("Data", "site1", "1_3_rast.tif"))
names(site_1_3) <- c("red", "green", "blue")
site_1_4 <- rast(here("Data", "site1", "1_4_rast.tif"))
names(site_1_4) <- c("red", "green", "blue")
site_1_5 <- rast(here("Data", "site1", "1_5_rast.tif"))
names(site_1_5) <- c("red", "green", "blue")
```

Import all polygons from site 1. Each sub-site contains 5 polygons of each class. 
```{r load polygons}
poly_1_1 <- vect(here("Data", "site1", "1_1_poly.shp"))
poly_1_2 <- vect(here("Data", "site1", "1_2_poly.shp"))
poly_1_3 <- vect(here("Data", "site1", "1_3_poly.shp"))
poly_1_4 <- vect(here("Data", "site1", "1_4_poly.shp"))
poly_1_5 <- vect(here("Data", "site1", "1_5_poly.shp"))
```
...
polygons <- list(
  poly_1_1 = "1_1_poly.shp",
  poly_1_2 = "1_2_poly.shp",
  poly_1_3 = "1_3_poly.shp",
  poly_1_4 = "1_4_poly.shp",
  poly_1_5 = "1_5_poly.shp"
)
```
...
for(name in names(polygons)) {
  poly <- vect(here("Data", "site1", polygons[[name]])) #get sub-site polygons
  ...
  subsite_number <- sub("poly_1_", "", name)  # Extract the subsite number
  points_filename <- paste0("1_", subsite_number, "_points.shp") #create filename for the new points dataset
  writeVector(all_randomPts, here("Data", "site1", points_filename), filetype = "ESRI Shapefile") #export the point dataset
...
```{r raster list}
#Create a list containing the orthomosaics of each subplot
rasters <- list(
  site_1_1 = site_1_1,
  site_1_2 = site_1_2,
  site_1_3 = site_1_3,
...
```{r points list}
# Create a list containing the randomly generated points at each subplot
points <- list(
...
for(raster_name in names(rasters)) {
  ...
  if (points_name %in% names(points)) { #identify if the file contains the corresponding subplot name in the points list
    ...
    writeVector(point_data, here("Data", "site1", gpkg_filename), filetype = "GPKG") #exports the geopackage file
