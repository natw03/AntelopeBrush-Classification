...
```{r}
pts_1_4 <- read_sf(here("Data", "site1", "1_4_pts_rgb.gpkg"), stringsAsFactors = T)
pts_2_4 <- read_sf(here("Data", "site2", "2_4_pts_rgb.gpkg"), stringsAsFactors = T)
pts_3_4 <- read_sf(here("Data", "site3", "3_4_pts_rgb.gpkg"), stringsAsFactors = T)
pts_4_4 <- read_sf(here("Data", "site4", "4_4_pts_rgb.gpkg"), stringsAsFactors = T)
pts_5_4 <- read_sf(here("Data", "site5", "5_4_pts_rgb.gpkg"), stringsAsFactors = T)
pts_6_4 <- read_sf(here("Data", "site6", "6_4_pts_rgb.gpkg"), stringsAsFactors = T)
pts_7_4 <- read_sf(here("Data", "site7", "7_4_pts_rgb.gpkg"), stringsAsFactors = T)
```

Select only the columns we want, which are className, red, green, blue and geom. 
```{r}
#Create a list containing all the data
trainSet4_list <- list(pts_1_4, pts_2_4, pts_3_4, pts_4_4, pts_5_4, pts_6_4, pts_7_4)
...
trainSet4_list <- lapply(trainSet4_list, select_columns) 

#Merge all datasets to create the full training dataset
trainSet4 <- do.call(bind_rows, trainSet1_list)
trainSet4
```
# Create model
...
```{r}
trainSet4_tsk <- as_task_classif_st(trainSet4, target = "className")
```

Partition the data into training and testing sets 
```{r}
set.seed(100)
trainSet4_split <- partition(trainSet4_tsk, ratio = 0.7)
```

Create random forest learner
```{r}
trainSet4_lrn <- lrn("classif.ranger")
```

Train the learner
```{r}
trainSet4_train <- trainSet4_lrn$train(trainSet4_tsk, trainSet4_split$train)
```

Test the learner's accuracy
```{r}
trainSet4_test <- trainSet4_lrn$predict(trainSet4_tsk, trainSet4_split$test)
trainSet4_test$score(msr("classif.acc"))
```
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_1 <- predict_spatial(raster_to_classify_1, trainSet4_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_2 <- predict_spatial(raster_to_classify_2, trainSet4_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_3 <- predict_spatial(raster_to_classify_3, trainSet4_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_4 <- predict_spatial(raster_to_classify_4, trainSet4_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_5 <- predict_spatial(raster_to_classify_5, trainSet4_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_6 <- predict_spatial(raster_to_classify_6, trainSet4_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_7 <- predict_spatial(raster_to_classify_7, trainSet4_lrn)
