...
```{r}
pts_1_5 <- read_sf(here("Data", "site1", "1_5_pts_rgb.gpkg"), stringsAsFactors = T)
pts_2_5 <- read_sf(here("Data", "site2", "2_5_pts_rgb.gpkg"), stringsAsFactors = T)
pts_3_5 <- read_sf(here("Data", "site3", "3_5_pts_rgb.gpkg"), stringsAsFactors = T)
pts_4_5 <- read_sf(here("Data", "site4", "4_5_pts_rgb.gpkg"), stringsAsFactors = T)
pts_5_5 <- read_sf(here("Data", "site5", "5_5_pts_rgb.gpkg"), stringsAsFactors = T)
pts_6_5 <- read_sf(here("Data", "site6", "6_5_pts_rgb.gpkg"), stringsAsFactors = T)
pts_7_5 <- read_sf(here("Data", "site7", "7_5_pts_rgb.gpkg"), stringsAsFactors = T)
```

Select only the columns we want, which are className, red, green, blue and geom. 
```{r}
#Create a list containing all the data
trainSet5_list <- list(pts_1_5, pts_2_5, pts_3_5, pts_4_5, pts_5_5, pts_6_5, pts_7_5)
...
trainSet5_list <- lapply(trainSet5_list, select_columns) 

#Merge all datasets to create the full training dataset
trainSet5 <- do.call(bind_rows, trainSet5_list)
trainSet5
```
# Create model
...
```{r}
trainSet5_tsk <- as_task_classif_st(trainSet5, target = "className")
```

Partition the data into training and testing sets 
```{r}
set.seed(100)
trainSet5_split <- partition(trainSet5_tsk, ratio = 0.7)
```

Create random forest learner
```{r}
trainSet5_lrn <- lrn("classif.ranger")
```

Train the learner
```{r}
trainSet5_train <- trainSet5_lrn$train(trainSet5_tsk, trainSet5_split$train)
```

Test the learner's accuracy
```{r}
trainSet5_test <- trainSet5_lrn$predict(trainSet5_tsk, trainSet5_split$test)
trainSet5_test$score(msr("classif.acc"))
```
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_1 <- predict_spatial(raster_to_classify_1, trainSet5_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_2 <- predict_spatial(raster_to_classify_2, trainSet5_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_3 <- predict_spatial(raster_to_classify_3, trainSet5_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_4 <- predict_spatial(raster_to_classify_4, trainSet5_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_5 <- predict_spatial(raster_to_classify_5, trainSet5_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_6 <- predict_spatial(raster_to_classify_6, trainSet5_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_7 <- predict_spatial(raster_to_classify_7, trainSet5_lrn)
