...
```{r split task}
set.seed(100)
#Partition the dataset into 70% for training the model and 30% for testing it
...
Train the learner
```{r train}
#train the random forest learner using the 70% partition of training data
trainSet1_train <- trainSet1_lrn$train(trainSet1_tsk, trainSet1_split$train)
```

Test the learner's accuracy
```{r test}
#evaluate the accuracy of the learner using the 30% partition of testing data
...
```{r rename layers}
subsite_ids <- 2:5 #only look at rasters with the subplot ID of 2 to 5
for (subsite_id in subsite_ids) {
  #rename the layers of the corresponding raster 
  raster <- get(paste0("rast_1_", subsite_id)) 
  names(raster) <- c("red", "green", "blue")
  #save the raster with the new layer names as rast_1_subplotID
...
```{r acc vect}
#create a vector object to store classification accuracies, with the number of rows being the number of subsite_ids 
...
Note: from this point, all code is structured the same as above and thus no detailed annotations were included
