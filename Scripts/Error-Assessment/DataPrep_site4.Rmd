...
Import all rasters from site 4
```{r load rasters}
site_4_1 <- rast(here("Data", "site4", "4_1_rast.tif"))
names(site_4_1) <- c("red", "green", "blue")
site_4_2 <- rast(here("Data", "site4", "4_2_rast.tif"))
names(site_4_2) <- c("red", "green", "blue")
site_4_3 <- rast(here("Data", "site4", "4_3_rast.tif"))
names(site_4_3) <- c("red", "green", "blue")
site_4_4 <- rast(here("Data", "site4", "4_4_rast.tif"))
names(site_4_4) <- c("red", "green", "blue")
site_4_5 <- rast(here("Data", "site4", "4_5_rast.tif"))
names(site_4_5) <- c("red", "green", "blue")
```

Import all polygons from site 4. Each sub-site contains 5 polygons of each class. 
```{r load polygons}
poly_4_1 <- vect(here("Data", "site4", "4_1_poly.shp"))
poly_4_2 <- vect(here("Data", "site4", "4_2_poly.shp"))
poly_4_3 <- vect(here("Data", "site4", "4_3_poly.shp"))
poly_4_4 <- vect(here("Data", "site4", "4_4_poly.shp"))
poly_4_5 <- vect(here("Data", "site4", "4_5_poly.shp"))
```
...
polygons <- list(
  poly_4_1 = "4_1_poly.shp",
  poly_4_2 = "4_2_poly.shp",
  poly_4_3 = "4_3_poly.shp",
  poly_4_4 = "4_4_poly.shp",
  poly_4_5 = "4_5_poly.shp"
)
```
...
for(name in names(polygons)) {
  poly <- vect(here("Data", "site4", polygons[[name]])) #get sub-site polygons
  ...
  subsite_number <- sub("poly_4_", "", name)  # Extract the subsite number
  points_filename <- paste0("4_", subsite_number, "_points.shp") #create filename for the new points dataset
  writeVector(all_randomPts, here("Data", "site4", points_filename), filetype = "ESRI Shapefile")
...
for(raster_name in names(rasters)) {
  ...
  if (points_name %in% names(points)) { #identify if the file contains the corresponding subplot name in the points list
    ...
    writeVector(point_data, here("Data", "site4", gpkg_filename), filetype = "GPKG") #exports the geopackage file
