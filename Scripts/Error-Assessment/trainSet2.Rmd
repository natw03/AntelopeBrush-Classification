...
```{r}
pts_1_2 <- read_sf(here("Data", "site1", "1_2_pts_rgb.gpkg"), stringsAsFactors = T)
pts_2_2 <- read_sf(here("Data", "site2", "2_2_pts_rgb.gpkg"), stringsAsFactors = T)
pts_3_2 <- read_sf(here("Data", "site3", "3_2_pts_rgb.gpkg"), stringsAsFactors = T)
pts_4_2 <- read_sf(here("Data", "site4", "4_2_pts_rgb.gpkg"), stringsAsFactors = T)
pts_5_2 <- read_sf(here("Data", "site5", "5_2_pts_rgb.gpkg"), stringsAsFactors = T)
pts_6_2 <- read_sf(here("Data", "site6", "6_2_pts_rgb.gpkg"), stringsAsFactors = T)
pts_7_2 <- read_sf(here("Data", "site7", "7_2_pts_rgb.gpkg"), stringsAsFactors = T)
```

Select only the columns we want, which are className, red, green, blue and geom. 
```{r}
#Create a list containing all the data
trainSet2_list <- list(pts_1_2, pts_2_2, pts_3_2, pts_4_2, pts_5_2, pts_6_2, pts_7_2)
...
trainSet2_list <- lapply(trainSet2_list, select_columns) 

#Merge all datasets to create the full training dataset
trainSet2 <- do.call(bind_rows, trainSet2_list)
trainSet2
```
# Create model
...
```{r}
trainSet2_tsk <- as_task_classif_st(trainSet2, target = "className")
```

Partition the data into training and testing sets 
```{r}
set.seed(100)
trainSet2_split <- partition(trainSet2_tsk, ratio = 0.7)
```

Create random forest learner
```{r}
trainSet2_lrn <- lrn("classif.ranger")
```

Train the learner
```{r}
trainSet2_train <- trainSet2_lrn$train(trainSet2_tsk, trainSet2_split$train)
```

Test the learner's accuracy
```{r}
trainSet2_test <- trainSet2_lrn$predict(trainSet2_tsk, trainSet2_split$test)
trainSet2_test$score(msr("classif.acc"))
```
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_1 <- predict_spatial(raster_to_classify_1, trainSet2_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_2 <- predict_spatial(raster_to_classify_2, trainSet2_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_3 <- predict_spatial(raster_to_classify_3, trainSet2_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_4 <- predict_spatial(raster_to_classify_4, trainSet2_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_5 <- predict_spatial(raster_to_classify_5, trainSet2_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_6 <- predict_spatial(raster_to_classify_6, trainSet2_lrn)
...
for (i in seq_along(subsite_ids)) {
  ...
  classified_raster_7 <- predict_spatial(raster_to_classify_7, trainSet2_lrn)
