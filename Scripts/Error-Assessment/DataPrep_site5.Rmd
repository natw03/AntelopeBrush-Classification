...
Import all rasters from site 5
```{r load rasters}
site_5_1 <- rast(here("Data", "site5", "5_1_rast.tif"))
names(site_5_1) <- c("red", "green", "blue")
site_5_2 <- rast(here("Data", "site5", "5_2_rast.tif"))
names(site_5_2) <- c("red", "green", "blue")
site_5_3 <- rast(here("Data", "site5", "5_3_rast.tif"))
names(site_5_3) <- c("red", "green", "blue")
site_5_4 <- rast(here("Data", "site5", "5_4_rast.tif"))
names(site_5_4) <- c("red", "green", "blue")
site_5_5 <- rast(here("Data", "site5", "5_5_rast.tif"))
names(site_5_5) <- c("red", "green", "blue")
```

Import all polygons from site 2. Each sub-site contains 5 polygons of each class. 
```{r load polygons}
poly_5_1 <- vect(here("Data", "site5", "5_1_poly.shp"))
poly_5_2 <- vect(here("Data", "site5", "5_2_poly.shp"))
poly_5_3 <- vect(here("Data", "site5", "5_3_poly.shp"))
poly_5_4 <- vect(here("Data", "site5", "5_4_poly.shp"))
poly_5_5 <- vect(here("Data", "site5", "5_5_poly.shp"))
```
...
polygons <- list(
  poly_5_1 = "5_1_poly.shp",
  poly_5_2 = "5_2_poly.shp",
  poly_5_3 = "5_3_poly.shp",
  poly_5_4 = "5_4_poly.shp",
  poly_5_5 = "5_5_poly.shp"
)
```
...
for(name in names(polygons)) {
  poly <- vect(here("Data", "site5", polygons[[name]])) #get sub-site polygons
  ...
  subsite_number <- sub("poly_5_", "", name)  # Extract the subsite number
  points_filename <- paste0("5_", subsite_number, "_points.shp") #create filename for the new points dataset
  writeVector(all_randomPts, here("Data", "site5", points_filename), filetype = "ESRI Shapefile")
...
for(raster_name in names(rasters)) {
  ...
  if (points_name %in% names(points)) { #identify if the file contains the corresponding subplot name in the points list
    ...
    writeVector(point_data, here("Data", "site5", gpkg_filename), filetype = "GPKG") #exports the geopackage file
